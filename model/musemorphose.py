...
# x와 seg_emb가 들어가면 out이 나옴.
# x는 입력 sequence, seg_emb는 condition, out은 L번째 hidden layer.
# In-attention 방식이 구현되어 있음. c를 projection시켜 차원을 d_model로 바꾼 후, 매 layer마다 이를 더해준다.
...
class MuseMorphose(nn.Module):
  ...
  # enc_inp -> vae_latent -> dec_seg_emb
  ...
  def forward(self, enc_inp, dec_inp, dec_inp_bar_pos, attr_cls_list=None, padding_mask=None):
    ...
    # 인코더랑 디코더 인풋
    enc_inp = self.emb_dropout(enc_token_emb) + self.pe(enc_inp.size(0))    # pe: positional encoding
    ...
    # 인코더에 인풋 넣으면, mean과 log(var)가 나옴.
    _, mu, logvar = self.encoder(enc_inp, padding_mask=padding_mask)
    vae_latent = self.reparameterize(mu, logvar)    # Reparametrization trick. Epsilon 뽑아서 만든 것.
